...
Cypress.Commands.add('login', user => {
  cy.request({
    ...
    window.localStorage.setItem(
      'userInfo',
      JSON.stringify(response.body.userInfo),
    )
...
Cypress.Commands.add('addFlashcard', ({term, definition, index}) => {
  ...
  cy.findByTestId(term ?? new RegExp(`term-${index}`)).type(
    `test-term-${index + 1}`,
  )
  cy.findByTestId(definition ?? RegExp(`definition-${index}`)).type(
    `test-definition-${index + 1}`,
  )
})
