    ...
    return (
      <div key={idx} className="w-full shadow-xl my-2 bg-white">
        ...
        <div className="flex w-full pt-2 pb-8">
          <div className="w-1/2 my-6 mr-6 pl-4" key={`field-term-${idx}`}>
            <TextBox
              // required={true}
              ...
              testId={`term-${idx}`}
            />
            <label htmlFor={`term-${idx}`} className="text-xs opacity-50 mt-1">
              TERM
            </label>
          </div>
          <div className="w-1/2 my-6 ml-6 pr-4" key={`field-answer-${idx}`}>
            <TextBox
              // required={true}
